    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <div className="invoice-receipt mx-auto bg-white shadow-sm my-6 px-4 py-5">
        ...
        <div className="text-xs space-y-0.5 mb-2">
          <p><span className="font-medium">Tracking:</span> {order.tracking_code}</p>
          <p><span className="font-medium">Date:</span> {new Date(order.created_at).toLocaleString()}</p>
          <p><span className="font-medium">Status:</span> {order.status.replace(/_/g, ' ')}</p>
        </div>
        ...
        <p className="text-center text-xs text-slate-500">Thank you for your order!</p>
      </div>
    </div>
  );
}
