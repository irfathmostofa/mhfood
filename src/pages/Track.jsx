...
export default function Track() {
  ...
  async function lookupOrder(trackingCode) {
    ...
    if (orderError || !orderData) {
      setError('No order found with that tracking code. Please check and try again.');
      setLoading(false);
      return;
    }
      ...
      <main className="max-w-2xl mx-auto px-5 py-12">
        {justPlaced && (
          <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm rounded-xl px-5 py-4 mb-8">
            Your order has been placed! A confirmation email is on its way — save this tracking
            code to check your order status anytime.
          </div>
        )}
        ...
        {loading && <p className="text-sm text-[#8A8578]">Looking up your order...</p>}
          ...
          <div className="bg-white border border-[#E7E0D3] rounded-2xl p-6 sm:p-8">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-lg font-semibold text-[#1F2A24]">{order.tracking_code}</p>
              </div>
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <p
                          className={`text-[9px] sm:text-xs mt-1.5 sm:mt-2 text-center w-12 sm:w-20 leading-tight ${
                            i <= currentStepIndex ? 'text-[#1F2A24] font-medium' : 'text-[#8A8578]'
                          }`}
                        >
                          {step.label}
                        </p>
                      ...
                      {i < STEPS.length - 1 && (
                        <div
                          className={`flex-1 h-0.5 mx-0.5 sm:mx-1 mt-3 sm:mt-4 ${
                            i < currentStepIndex ? 'bg-[#1F2A24]' : 'bg-[#F3EEE2]'
                          }`}
                        />
                      )}
                    </div>
            ...
            <div className="border-t border-[#E7E0D3] pt-5">
              ...
              <ul className="space-y-2 mb-5">
                {items.map((item) => (
                  <li key={item.id} className="flex justify-between text-sm text-[#4A5049]">
                    <span>
            ...
            <div className="border-t border-[#E7E0D3] mt-5 pt-5 text-sm text-[#4A5049]">
              <p className="font-medium text-[#1F2A24] mb-1">Delivery Address</p>
              <p>{order.address}</p>
