...
export default function ProductPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setProduct(prod);
      setImages([...(prod.product_images || [])].sort((a, b) => a.sort_order - b.sort_order));

      const [{ data: rating }, { data: reviewRows }, { data: countRow }] = await Promise.all([
        supabase.from('product_ratings').select('*').eq('product_id', prod.id).maybeSingle(),
        supabase
          .from('reviews')
          .select('*')
        ...
        supabase.from('product_order_counts').select('*').eq('product_id', prod.id).maybeSingle(),
      ]);
    ...
    return (
      ...
      <BackToTop />
        <div className="text-center py-24">
          ...
          <p className="text-sm text-[#8A8578]">It may be unavailable or no longer exists.</p>
        </div>
      </div>
    );
  }
                ...
                {images.map((img, i) => (
                  <button
                    ...
                    className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors ${
                      i === activeImage ? 'border-[#1F2A24]' : 'border-transparent'
                    }`}
                  >
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-sm text-[#8A8578]">
                {ratingInfo.review_count} review{ratingInfo.review_count === 1 ? '' : 's'}
              </span>
              {orderCount > 0 && (
                <span className="text-sm text-[#8A8578]">· {orderCount} sold</span>
              )}
            </div>

            <p className="text-2xl font-semibold text-[#C77B4C] mb-4">৳{product.price}</p>

            {product.description && (
              <p className="text-sm text-[#4A5049] leading-relaxed mb-6">{product.description}</p>
            )}
            ...
            {product.stock === 0 ? (
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-xs text-[#8A8578]">{product.stock} in stock</span>
              </div>
            )}
