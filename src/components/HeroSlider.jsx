...
export default function HeroSlider() {
  ...
  return (
    <div className="relative w-full h-[320px] sm:h-[420px] overflow-hidden rounded-2xl bg-[#EFE8D8]">
      {slides.map((s, i) => (
        <div
          key={s.id}
          ...
          {(s.title || s.subtitle) && (
            <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-black/10 to-transparent flex items-end">
              <div className="p-6 sm:p-10 text-white max-w-lg">
                ...
                {s.subtitle && <p className="text-sm sm:text-base text-white/90">{s.subtitle}</p>}
                {s.link_url && (
                  <a
                    href={s.link_url}
