...
// Sends the SAME message to many customers at once — one request to our
// function, which forwards a comma-separated number list to BulkSMSBD.
export async function sendBulkSMS({ phones, message }) {
  const numbers = [...new Set(phones.map(normalizeBdNumber))];
